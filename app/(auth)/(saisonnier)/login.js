  ...
  const handleLogin = useCallback(async() => {
    try{
      const user = {
       ...
       await axios.post(`http://localhost:8002/login-saisonnier`, user)
        .then((res) => {
          const token = res.data.token;
          AsyncStorage.setItem("authToken", token);
          AsyncStorage.setItem("authType", "saisonnier");
          const decoded = jwtDecode(token);
          const loginId = decoded.userId;
          setAuthType("saisonnier");
          setUserId(loginId);
          setEmail("");
          setPassword("");
        })
      ...
      <View style={styles.logoContainer}>
        ...
        <View style={{ justifyContent: "flex-start",gap: 5,}}>
          ...
          <View style={{flexDirection:"row", alignItems: "center"}}>
            <Text>Don't have an Saisonnier account? </Text>
          <Pressable onPress={() => router.replace("/(auth)/(saisonnier)/(register)")}>
