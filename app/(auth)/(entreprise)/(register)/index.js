...
const register = () => {
    ...
    const handleRegister = async () => {
        ...
        await axios.post('http://localhost:8002/register-entreprise', user)
        .then((res)=>{
                console.log(res);
  ...
  return (
    <SafeAreaView style={{flex:1, backgroundColor:"white", paddingHorizontal: 15}}>
        <View style={styles.logoContainer}>
            ...
            />
          <Text style={styles.titleText}>Sign up on Entreprise account</Text>
          <View style={{flexDirection:"row", alignItems: "center"}}>
            <Text>or </Text>
          <Pressable onPress={() => router.replace("/(auth)/(entreprise)/login")}>
